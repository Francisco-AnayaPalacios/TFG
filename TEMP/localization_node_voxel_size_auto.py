...
class LocalizationNode(Node):
    def __init__(self, voxel_size, done_event):
        super().__init__('localization_node')
        self.done_event = done_event 
        print(f"Inicializando el nodo de localización con voxelización de {voxel_size}...")

        self.voxel_size = voxel_size
        self.save_path = f"/odom_data/odom_data_{voxel_size}_voxel.csv"  # Ruta al fichero de datos de salida
        # MAPA 1
        self.rosbag_path = "/rosbags/rosbag1_fl"  # Ruta a la rosbag
        self.map_file = "/MAPAS/mapa1.pcd"  # Ruta al mapa
    ...
    def points_callback(self, msg):  # Procesa los mensajes de nubes de puntos del LiDAR, se lanza ICP y se guardan los valores de interés
        ...
        if self.enable_voxelization:
            original_cloud = original_cloud.voxel_down_sample(self.voxel_size)
    ...
    def perform_icp(self, source_cloud, target_cloud):
        print(f"Calculando ICP con voxelización de {self.voxel_size}")
        threshold = 1.5
        ...
        reg_p2p = o3d.pipelines.registration.registration_icp(
            ...
            o3d.pipelines.registration.ICPConvergenceCriteria(
                max_iteration=50  # Usar un valor fijo de iteraciones para ICP
            )
        )
        icp_end = time.time()
...
def main(args=None):
    voxel_sizes = [0.01,0.1]  # Lista de tamaños de voxelización a probar

    for voxel_size in voxel_sizes:
        rclpy.init(args=args)
        done_event = Event()
        node = LocalizationNode(voxel_size, done_event)
        def check_progress():
            while rclpy.ok():
