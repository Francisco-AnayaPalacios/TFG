...
class LocalizationNode(Node):
    def __init__(self):
        ...
        print("Inicializando el nodo de localización...")
        
        self.save_path = "/odom_data/odom_data.csv"  # Ruta al fichero de datos de salida
        # MAPA 1
    ...
    def resume_bag(self):
        if self.bag_process and self.bag_paused:
            ...
            self.bag_paused = False
            self.last_resume_time = time.time()  # Reinicia la cuenta de tiempo con la reproducción de la rosbag activa
    ...
    def perform_icp(self, source_cloud, target_cloud):
        ...
        # Variando la escala de la transformación de grande a precisa
        #reg_multiscale = o3d.pipelines.registration.registration_multi_scale_icp(
        #    source_cloud, target_cloud, [threshold], [trans_init],
        #    o3d.pipelines.registration.ICPConvergenceCriteria(relative_fitness=1e-6, relative_rmse=1e-6, max_iteration=1000)
        #)
        # De punto a plano
        #reg_p2p = o3d.pipelines.registration.registration_icp(
        #    source_cloud, target_cloud, threshold, trans_init,
    ...
    def get_bag_progress(self):
        if self.rosbag_duration > 0:
            ...
            if progress_percentage >= 70*100:
                print("Rosbag completada.")
                self.bag_process.send_signal(signal.SIGTERM)  
                rclpy.shutdown()  
            else:
